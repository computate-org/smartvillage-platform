...
public class VehicleStepEnUSGenApiServiceImpl extends BaseApiServiceImpl implements VehicleStepEnUSGenApiService {
	...
	public VehicleStepEnUSGenApiServiceImpl(EventBus eventBus, JsonObject config, WorkerExecutor workerExecutor, PgPool pgPool, WebClient webClient, OAuth2Auth oauth2AuthenticationProvider, AuthorizationProvider authorizationProvider, HandlebarsTemplateEngine templateEngine) {
	...
	public void searchVehicleStep(ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "smart-village-view-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				List<String> roles = Optional.ofNullable(config.getValue(ComputateConfigKeys.AUTH_ROLES_REQUIRED + "_VehicleStep")).map(v -> v instanceof JsonArray ? (JsonArray)v : new JsonArray(v.toString())).orElse(new JsonArray()).getList();
	...
	public Future<Void> listPATCHVehicleStep(ApiRequest apiRequest, SearchList<VehicleStep> listVehicleStep) {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			if(apiRequest != null) {
				...
				if(apiRequest.getNumFound() == 1L)
					listVehicleStep.first().apiRequestVehicleStep();
				eventBus.publish("websocketVehicleStep", JsonObject.mapFrom(apiRequest).toString());
			}
		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "smart-village-view-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				if(
					...
					eventBus.request("smart-village-view-enUS-VehicleStep", json, new DeliveryOptions().addHeader("action", "postVehicleStepFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smart-village-view-enUS-VehicleStep", json, new DeliveryOptions().addHeader("action", "putimportVehicleStepFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public String templateSearchPageVehicleStep() {
		return Optional.ofNullable(config.getString(ComputateConfigKeys.TEMPLATE_PATH)).orElse("templates") + "/enUS/VehicleStepPage";
	}
