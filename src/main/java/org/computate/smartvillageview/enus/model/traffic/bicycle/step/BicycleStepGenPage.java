...
public class BicycleStepGenPage extends BicycleStepGenPageGen<MapResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		BicycleStep.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListBicycleStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(BicycleStep.varIndexedBicycleStep(var) + ":")).map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		BicycleStep.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(BicycleStep.displayNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(BicycleStep.classSimpleNameBicycleStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListBicycleStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BicycleStep.varIndexedBicycleStep(var) + ":")).findFirst().map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
	...
	protected void _defaultRangeGap(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeGap()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> s.getString("defaultRangeGap")).orElse("+1DAY")));
	}

	@Override
	protected void _defaultRangeEnd(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeEnd")).atZone(defaultTimeZone)).orElse(ZonedDateTime.now(defaultTimeZone).toLocalDate().atStartOfDay(defaultTimeZone).plusDays(1))));
	}

	@Override
	protected void _defaultRangeStart(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListBicycleStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeStart")).atZone(defaultTimeZone)).orElse(defaultRangeEnd.minusDays(7).toLocalDate().atStartOfDay(defaultTimeZone))));
	}
	...
	protected void _rolesRequired(List<String> l) {
		l.addAll(Optional.ofNullable(siteRequest_.getConfig().getJsonArray(ConfigKeys.AUTH_ROLES_REQUIRED + "_BicycleStep")).orElse(new JsonArray()).stream().map(o -> o.toString()).collect(Collectors.toList()));
	}
