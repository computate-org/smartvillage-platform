...
public class SitePageGenPage extends SitePageGenPageGen<BaseResultPage> {
	...
	protected void _rolesRequired(List<String> l) {
		l.addAll(Optional.ofNullable(siteRequest_.getConfig().getJsonArray(ComputateConfigKeys.AUTH_ROLES_REQUIRED + "_SitePage")).orElse(new JsonArray()).stream().map(o -> o.toString()).collect(Collectors.toList()));
	}
