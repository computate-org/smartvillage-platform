...
public class TrafficFlowObservedGenPage extends TrafficFlowObservedGenPageGen<MapResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		TrafficFlowObserved.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListTrafficFlowObserved_.getRequest().getQuery()).filter(fq -> fq.startsWith(TrafficFlowObserved.varIndexedTrafficFlowObserved(var) + ":")).map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TrafficFlowObserved.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(TrafficFlowObserved.displayNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficFlowObserved.classSimpleNameTrafficFlowObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTrafficFlowObserved_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TrafficFlowObserved.varIndexedTrafficFlowObserved(var) + ":")).findFirst().map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
