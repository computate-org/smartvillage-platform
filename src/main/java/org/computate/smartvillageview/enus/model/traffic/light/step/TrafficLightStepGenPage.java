...
public class TrafficLightStepGenPage extends TrafficLightStepGenPageGen<MapResultPage> {
	...
	protected void _varsQ(JsonObject vars) {
		TrafficLightStep.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListTrafficLightStep_.getRequest().getQuery()).filter(fq -> fq.startsWith(TrafficLightStep.varIndexedTrafficLightStep(var) + ":")).map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		TrafficLightStep.varsFqForClass().forEach(var -> {
			...
			json.put("displayName", Optional.ofNullable(TrafficLightStep.displayNameTrafficLightStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("classSimpleName", Optional.ofNullable(TrafficLightStep.classSimpleNameTrafficLightStep(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
			json.put("val", searchListTrafficLightStep_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(TrafficLightStep.varIndexedTrafficLightStep(var) + ":")).findFirst().map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
	...
	protected void _defaultRangeGap(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTrafficLightStep_.getFacetRangeGap()).orElse("+1DAY"));
	}

	@Override
	protected void _defaultRangeEnd(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListTrafficLightStep_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(ZonedDateTime.now(defaultTimeZone).toLocalDate().atStartOfDay(defaultTimeZone).plusDays(1)));
	}

	@Override
	protected void _defaultRangeStart(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListTrafficLightStep_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(defaultRangeEnd.minusDays(7).toLocalDate().atStartOfDay(defaultTimeZone)));
	}

	@Override
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListTrafficLightStep_.getFacetRanges()).orElse(Arrays.asList()).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return TrafficLightStep.searchVarTrafficLightStep(v); }).orElse("created"));
	}
