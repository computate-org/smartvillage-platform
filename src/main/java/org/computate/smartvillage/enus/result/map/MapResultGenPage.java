...
import org.computate.smartvillage.enus.result.map.MapResultGenPageGen;
...
public class MapResultGenPage extends MapResultGenPageGen<BaseResultPage> {
	...
	protected void _roleForWrite(List<String> l) {
		Optional.ofNullable(siteRequest_.getConfig().getString(ConfigKeys.AUTH_ROLE_REQUIRED + "_MapResult")).ifPresent(v -> {
			l.add(v);
		});
	}
