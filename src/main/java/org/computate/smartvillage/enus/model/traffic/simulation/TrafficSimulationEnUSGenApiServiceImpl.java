			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request("smartvillage-platform-enUS-TrafficSimulation", json, new DeliveryOptions().addHeader("action", "postTrafficSimulationFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smartvillage-platform-enUS-TrafficSimulation", json, new DeliveryOptions().addHeader("action", "putimportTrafficSimulationFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
