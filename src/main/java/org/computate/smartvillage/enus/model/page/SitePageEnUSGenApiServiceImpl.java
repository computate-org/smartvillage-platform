			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request("smartvillage-platform-enUS-SitePage", json, new DeliveryOptions().addHeader("action", "postSitePageFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smartvillage-platform-enUS-SitePage", json, new DeliveryOptions().addHeader("action", "putimportSitePageFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
