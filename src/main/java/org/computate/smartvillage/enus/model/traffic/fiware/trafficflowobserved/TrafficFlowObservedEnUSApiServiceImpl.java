						...
						if("SmartTrafficLight".equals(classSimpleName2) && id2 != null) {
							...
							searchList2.promiseDeepSearchList(siteRequest).onSuccess(c -> {
								...
								if(o2 != null) {
									...
									eventBus.request("smartvillage-platform-enUS-SmartTrafficLight", json, new DeliveryOptions().addHeader("action", "patchSmartTrafficLightFuture")).onSuccess(d -> {
										...
										if(statusCode.equals(200)) {
											...
											for(SimulationReport o3 : searchReportList2.getList()) {
												...
												futures.add(Future.future(promise2 -> {
													...
													eventBus.request("smartvillage-platform-enUS-SimulationReport", json3, new DeliveryOptions().addHeader("action", "patchSimulationReportFuture")).onSuccess(e -> {
											...
											CompositeFuture.all(futures).onSuccess(e -> {
												...
												JsonObject context3 = new JsonObject().put("params", params3).put("user", siteRequest.getUserPrincipal());
												JsonObject json3 = new JsonObject().put("context", context3);
												eventBus.request("smartvillage-platform-enUS-TrafficFlowObserved", json3, new DeliveryOptions().addHeader("action", "patchTrafficFlowObservedFuture")).onSuccess(f -> {
