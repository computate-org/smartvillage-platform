			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request("smartvillage-platform-enUS-TrafficLightStep", json, new DeliveryOptions().addHeader("action", "postTrafficLightStepFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smartvillage-platform-enUS-TrafficLightStep", json, new DeliveryOptions().addHeader("action", "putimportTrafficLightStepFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
