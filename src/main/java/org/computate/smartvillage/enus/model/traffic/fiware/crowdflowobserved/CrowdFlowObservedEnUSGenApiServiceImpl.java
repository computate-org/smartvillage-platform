			...
			authorizationProvider.getAuthorizations(siteRequest.getUser()).onFailure(ex -> {
				...
				if(
					...
					try {
						...
						eventBus.request("smartvillage-platform-enUS-CrowdFlowObserved", json, new DeliveryOptions().addHeader("action", "postCrowdFlowObservedFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smartvillage-platform-enUS-CrowdFlowObserved", json, new DeliveryOptions().addHeader("action", "putimportCrowdFlowObservedFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
		...
		try {
			...
			if(refresh && !Optional.ofNullable(siteRequest.getJsonObject()).map(JsonObject::isEmpty).orElse(true)) {
				...
				CompositeFuture.all(futures).onSuccess(b -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("smartvillage-platform-enUS-CrowdFlowObserved", json, new DeliveryOptions().addHeader("action", "patchCrowdFlowObservedFuture")).onSuccess(c -> {
