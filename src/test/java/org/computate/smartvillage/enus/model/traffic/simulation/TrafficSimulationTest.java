...
//		Mockito.doCallRealMethod().when(mainVerticle).start(Mockito.any());
		...
		}).when(webClient).get(Mockito.anyInt(), Mockito.anyString(), Mockito.eq("/solr/smartvillage-platform/select?q=*%3A*&fq=userId_docvalues_string%3Atest&fq=classCanonicalNames_docvalues_strings%3Aorg.computate.smartvillage.enus.model.user.SiteUser&fq=deleted_docvalues_boolean%3Afalse&fq=archived_docvalues_boolean%3Afalse&rows=10&start=0"));
