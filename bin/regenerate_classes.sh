set -f
systemctl --user stop watch-smart-village-view

toilet -t -f smblock Run Base Automation
echo Install all of the base application code that is generic to most projects
echo ansible-playbook ~/.local/src/computate-org/vertx_project.yml -e @~/.local/src/smart-village-view/local/ansible_install_vars.yml
read -p "" < /dev/tty
ansible-playbook ~/.local/src/computate-org/vertx_project.yml -e @~/.local/src/smart-village-view/local/ansible_install_vars.yml

curl -s 'http://localhost:8983/solr/computate/select?fq=classSort_indexed_int%3A%5B*%20TO%20*%5D&fq=siteNom_indexed_string%3Asmart-village-view&fq=partEstClasse_indexed_boolean%3Atrue&q=*%3A*&sort=classSort_indexed_int%20asc&rows=100' \
| jq -c '.response.docs[]' \
| while IFS1=$'\n' read -r c; do 
	...
	PART_URL='http://localhost:8983/solr/computate/select?fq=siteNom_indexed_string%3Asmart-village-view&fq=-partEstClasse_indexed_boolean%3A%5B*%20TO%20*%5D&fq=classeNomSimple_enUS_indexed_string%3A'"$classeNomSimple_enUS_stored_string"'&q=*%3A*&sort=partNumero_indexed_int%20asc&rows=1000'
...
echo '(cd ~/.local/src/smart-village-view && ansible-playbook ~/.ansible/roles/computate.computate_project/install.yml -e @~/.local/src/smart-village-view/vault/$USER-vault --vault-password-file ~/.local/src/smart-village-view/vault/key -e SITE_NAME=smart-village-view -e ENABLE_CODE_GENERATION_SERVICE=true)'
read -p "" < /dev/tty
(cd ~/.local/src/smart-village-view && ansible-playbook ~/.ansible/roles/computate.computate_project/install.yml -e @~/.local/src/smart-village-view/vault/$USER-vault --vault-password-file ~/.local/src/smart-village-view/vault/key -e SITE_NAME=smart-village-view -e ENABLE_CODE_GENERATION_SERVICE=true)

toilet -t -f smblock Watch Service
echo Watch the watch-smart-village-view service logs as it generates the API and UI code
echo 'journalctl -f --user-unit watch-smart-village-view'
read -p "" < /dev/tty

toilet -t -f smblock Touch source code
echo With the watch-smart-village-view service running, we can simply touch all of the source code files we just wrote to generate the API and UI code. 
echo ~/.local/src/smart-village-view/bin/touch.sh
read -p "" < /dev/tty
~/.local/src/smart-village-view/bin/touch.sh
sleep 120
~/.local/src/smart-village-view/bin/touch.sh
